...
import { fadePageIn } from "./js/elements/fadePageIn.js"
import { setBodySize } from "./js/utils/setBodySize.js"
import { validateOptionals, validateFirstPage, validateSecondPage } from "./js/validation"
...
function setSessionData(selectedGenres, selectedProviders, sessionObject) {
	...
	sessionObject.country = country.value === "default" ? undefined : country.value

	validateOptionals(sessionObject)
	return sessionObject
}

async function submitSession(sessionObject) {
	...
	if (!result.error) {
		const moviesResult = await createMovieRecords(sessionObject.sessionName, movieArray)

		if (!moviesResult.error) {
			window.location.href = `app.html?session=${sessionObject.sessionName}`
		} else {
			console.log(moviesResult.error)
		}
...
function handleGenreSelections(e, selectedGenres, sessionObject) {
	...
	genreNames.innerText =
		selectedGenres.length === 0 ? "Required" : `${selectedGenres.length} selected`
	setSessionData(selectedGenres, selectedProviders, sessionObject)
}
...
function handleProviderSelections(e, selectedProviders, sessionObject) {
	...
	providerNames.innerText =
		selectedProviders.length === 0 ? "Optional" : `${selectedProviders.length} selected`
